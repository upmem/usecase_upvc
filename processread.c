...
int cmpcoord(const void * a, const void * b) {
	ALIGN *A = (ALIGN *) a;
	ALIGN *B = (ALIGN *) b;
	if (A->coord_seq == B->coord_seq) {
		if (A->coord_pos > B->coord_pos){
			return 1;
		}
		else if (A->coord_pos < B->coord_pos){
			return -1;
		}
		else return 0;
	}
...
void getSizeInsert (ALIGN * A, int nb_match) {
	...
	int total = 0, score[4], offset[4], nbread[4], numpair, type, count = 0, delta[MAX_ALIGN*4], d;
	double std = 0;
	...
	for (i = 0; i < nb_match; i++) {
		...
		for (pa = offset[0]; pa < offset[0] + nbread[0]; pa++)
			for (pb = offset[3]; pb < offset[3] + nbread[3]; pb++) {
				d = abs(A[pa].coord_pos - A[pb].coord_pos + SIZE_READ);
				if (d < SIZE_READ * 10) {
					delta[count] = d;
					total += d;
					count++;
	...
	SIZE_INSERT_MEAN = total/count;

	for (i = 0; i < count; i++) {
		std += pow(delta[i] - SIZE_INSERT_MEAN, 2);
	}

	SIZE_INSERT_STD = (int) sqrt(std/count);
...
void set_variant(ALIGN A, GENOME *RG, int8_t *BUF_READ, VARINDEL **INDEL,
		int *SUB, int8_t *MAPCOV) {
	...
	code_alignment(codali, A.score, &RG->data[pos],
			&BUF_READ[A.num_read * SIZE_READ]);
	...
	while (codali[i] != CODE_END) {
		...
		} else // variant de type indel : D/I pos x+
		{
			...
			indel = INDEL[ix];
			while (indel != NULL) {
				if ((indel->type == type) && (indel->pos_seq == ptsnp)
						&& (indel->num_seq == A.coord_seq)
						&& (indel->content == content)) {
					indel->count += 1;
					break;
				}
...
int process_read(GENOME *RG, int8_t *BUF_READ, int nb_read, VARINDEL **INDEL, int *SUB, int8_t *MAPCOV, TIMES *CT) 
{
  ...
  //ALIGN *B = (ALIGN *) malloc(sizeof(ALIGN) * MAX_ALIGN * NB_DPU);
  ...
  /*
	...
	qsort(A, nb_match, sizeof(ALIGN), cmpcoord);
	B[0] = A[0];
	int j = 0;
	for (i = 1; i < nb_match; i++){
		if (A[i].coord_seq != A[i-1].coord_seq || A[i].coord_pos != A[i-1].coord_pos) {
			B[j] = A[i];
			memcpy(&B[j], &A[i], sizeof(ALIGN));
			j++;
		}
	}
