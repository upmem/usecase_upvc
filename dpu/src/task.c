 ...
 * @var nr_reads   the number of reads in the request pool
...
/**
 * @return the transfer size needed to load a neighbor as a byte stream
 */
...
static void request_pool_init(mram_addr_t io_offs) {
    ...
    request_pool.nr_reads = io_data->nr_reads;
    ...
    // able to contain a neighbor and a request header, aligned on a multiple of
    // 8 bytes.
 ...
 * @param request filled with the next request, if found
 * @param nbr     filled with the requested neighbor, if found
...
static bool request_pool_next(request_t *request, uint8_t *nbr, dpu_tasklet_stats_t *stats) {
    ...
    if (request_pool.rdidx == request_pool.nr_reads) {
    ...
    (void) memcpy(request, request_pool.cache, sizeof(request_t));
    (void) memcpy(nbr, request_pool.cache + SIZEOF_REQUEST_HEADER, mram_info->nbr_len);
...
static void result_pool_write(const dout_t *results, dpu_tasklet_stats_t *stats) {
    ...
    for (pageno = 0; pageno < results->nr_page_out; pageno++) {
    ...
    for (i = 0; (result_pool.wridx < (MAX_DPU_RESULTS - 1)) && (i < results->nr_cached_out); i++) {
...
/**
 * @brief Fetches a neighbor from the neighbor area.
 * @param base    offset to the first neighbor of the requested pool within the neighbor area
 * @param idx     index of this neighbor in the specified pool
 * @param buffer  to contain the result, must be the size of a neighbor and coordinates, plus 64 bits alignment
 * @param stats   to update statistical report
 * @return A pointer to the beginning of the loaded neighbor
 */
static uint8_t *
load_reference_nbr_and_coords_at(unsigned int base, unsigned int idx, uint8_t *cache, dpu_tasklet_stats_t *stats) {
    // The input starts with coordinates (8 bytes), followed by the neighbor. Structure is aligned
    // on 8 bytes boundary.
...
static int run_align() {
    ...
    uint8_t *syms = mem_alloc(nr_bytes_to_syms(nbr_len_as_longs()));
#endif // DEBUG_REQUESTS
    ...
    while (request_pool_next(&request, current_read_nbr, &tasklet_stats)) {
        ...
        stat_inc_nr_reads(&stats);
        tasklet_stats.nr_reqs++;
        ...
        for (idx = 0; idx < request.count; idx++) {
            ...
            tasklet_stats.nr_nodp_calls++;
            stat_inc_nr_refs(&stats);
            ...
            if (score_nodp == -1) {
                stat_inc_nr_odpd_calls(&stats);
#ifndef OPT_S
                int score_odpd = odpd(current_read_nbr, ref_nbr, mini, nr_bytes_to_syms(mram_info->nbr_len), me());
#else
                int score_odpd = odpdx(current_read_nbr, ref_nbr, mini, nr_bytes_to_syms(mram_info->nbr_len), me());
#endif // OPT_S
                tasklet_stats.nr_odpd_calls++;
                score = score_odpd;
            ...
            if (score <= mini) {
                ...
                if (dout.nr_results < MAX_RESULTS_PER_READ) {
...
#ifdef DEBUG_RESULTS
                    printf("[%u] nr=%u ix=%u num=%u ", me(), debug_nr, idx, out->num);
                    printf("offset=%u seed=%u seq=%u score=%u\n", request.offset, out->seed_nr, out->seq_nr, score);
#endif // DEBUG_RESULTS
                } else {
        ...
        if (dout.nr_results != 0) {
            tasklet_stats.nr_results += dout.nr_results;
...
#ifdef OPT_S
        if (((nr_bytes_to_syms(mram_info->nbr_len) + 1) * 24) >= 4096) {
            printf("cannot run optimized code: symbol length is larger than mulub operation\n");
            halt();
        }
#endif // OPT_S
        odpd_init(NR_RUNNING_TASKLETS, nr_bytes_to_syms(mram_info->nbr_len));
    }
    barrier_wait(barrier);

    // For debugging purpose, one may reduce the number of operating tasklets.
    if (me() < NR_RUNNING_TASKLETS) return run_align(); else return 0;
}
