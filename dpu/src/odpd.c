...
static inline unsigned int nr_items_per_matrix(unsigned int nbr_sym_len) {
...
static inline unsigned int sizeof_matrix(unsigned int nbr_sym_len) {
    ...
    return nr_items_per_matrix(nbr_sym_len) << 2;
}

void odpd_init(unsigned int nr_tasklets, unsigned int nbr_sym_len) {
    __D = mem_alloc(sizeof_matrix(nbr_sym_len) * nr_tasklets);
    __P = mem_alloc(sizeof_matrix(nbr_sym_len) * nr_tasklets);
    __Q = mem_alloc(sizeof_matrix(nbr_sym_len) * nr_tasklets);
}
...
int odpd(const uint8_t *s1, const uint8_t *s2, int max_score, unsigned int nbr_sym_len, unsigned int tid) {
    ...
    for (j = 0; j <= NR_DIAGS / 2 + 1; j++) {
        P(0, j) = 99;
        Q(0, j) = 99;
    }
    P(1, 0) = 99;
    Q(1, 0) = 99;
    for (j = 0; j <= NR_DIAGS / 2 + 1; j++) { D(0, j) = j * COST_SUB; }

    for (i = 1; i < NR_DIAGS / 2 + 1; i++) {
        min_score = 99;
        pp = i & 1;     // i % 2
        lp = pp ^ 1;    // (i - 1) % 2
        D(pp, 0) = i * COST_SUB;
        for (j = 1; j <= i + NR_DIAGS / 2; j++) {
    ...
    for (i = NR_DIAGS / 2 + 1; i < nbr_sym_len - NR_DIAGS / 2; i++) {
        min_score = 99;
        pp = i & 1;     // i % 2
        lp = pp ^ 1;    // (i - 1) % 2
        j = i - NR_DIAGS / 2 - 1;
        P(pp, j) = 99;
        D(pp, j) = 99;
        for (j = i - NR_DIAGS / 2; j <= i + NR_DIAGS / 2; j++) {
            P(pp, j) = min(D(pp, j - 1) + COST_GAPO, P(pp, j - 1) + COST_GAPE);
            Q(pp, j) = min(D(lp, j) + COST_GAPO, Q(lp, j) + COST_GAPE);
            QP = min(P(pp, j), Q(pp, j));
            d = D(lp, j - 1);
    ...
    for (i = nbr_sym_len - NR_DIAGS / 2; i < nbr_sym_len + 1; i++) {
        pp = i & 1;     // i % 2
        lp = pp ^ 1;    // (i - 1) % 2
        j = i - NR_DIAGS / 2 - 1;
        P(pp, j) = 99;
        D(pp, j) = 99;
        for (j = i - NR_DIAGS / 2; j < nbr_sym_len + 1; j++) {
            P(pp, j) = min(D(pp, j - 1) + COST_GAPO, P(pp, j - 1) + COST_GAPE);
            Q(pp, j) = min(D(lp, j) + COST_GAPO, Q(lp, j) + COST_GAPE);
            QP = min(P(pp, j), Q(pp, j));
            d = D(lp, j - 1);
        ...
        if (D(pp, nbr_sym_len) < min_score) min_score = D(pp, nbr_sym_len);
    }
    i = nbr_sym_len;
    pp = i & 1;         // i % 2
    for (j = i - NR_DIAGS / 2; j < nbr_sym_len + 1; j++)
