...
#define STATS_GET_START_TIME(start, acc, end) do { start = perfcounter_get(); acc = 0ULL; end = start; } while(0)
#define STATS_GET_END_TIME(end, acc) do { get_time_and_accumulate(&acc, &end); } while(0)
#define STATS_STORE_NODP_TIME(stats_ptr, val) do { (stats_ptr)->nodp_time += (val); } while(0)
#define STATS_STORE_ODPD_TIME(stats_ptr, val) do { (stats_ptr)->odpd_time += (val); } while(0)
