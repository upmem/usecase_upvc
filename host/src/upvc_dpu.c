...
void write_neighbours_and_coordinates(int numdpu, int index_idx, int8_t *nbrs, dpu_result_coord_t coord, reads_info_t *reads_info)
{
        ...
        *(dpu_result_coord_t *)(mdpu->neighbour_idx + (index_idx * aligned_coord_nbr_len)) = coord;
        (void) memcpy(mdpu->neighbour_idx + (index_idx * aligned_coord_nbr_len) + sizeof(dpu_result_coord_t),
                      nbrs,
                      (size_t) aligned_nbr_len);
}
...
void print_neighbour_idx(int d, int offs, int nb_nbr, FILE *out, reads_info_t *reads_info)
{
        ...
        for (int each_nbr = 0; each_nbr < nb_nbr; each_nbr++) {
                ...
                for (i = 0; i < size_neighbour_in_bytes; i++) {
                        uint8_t this_byte =
                                (uint8_t)
                                MDPU[d].neighbour_idx[(offs + each_nbr) * aligned_coord_nbr_len + sizeof(dpu_result_coord_t) + i];
...
void print_coordinates(int d, int offs, int l, FILE *out, reads_info_t *reads_info)
{
        ...
        for (int i = 0; i < l; i++) {
                fprintf(out,
                        " %llu",
                        (unsigned long long)
                        ((dpu_result_coord_t *)(&MDPU[d].neighbour_idx[(offs + l) * aligned_coord_nbr_len]))->coord);
        }
        fprintf(out, "\n");
}
