...
parser = argparse.ArgumentParser(description='Compute VCF quality compare to reference VCF')
parser.add_argument("ref_file", metavar="<reference.vcf>", help="The reference VCF")
parser.add_argument("upvc_file", metavar="<upvc.vcf>", help="The VCF generated by UPVC")
parser.add_argument("-t", dest="tp_power", type=float, help="The power to use for TP during auto compute of UPVC filter", default = 2.0)
parser.add_argument("-p", dest="tp_fp_prop", type=float, help="A threshold for the proportion of TP and FP to use during auto compute of UPVC filter", default = 1.0)
parser.add_argument("-d", dest="score_dropback_threshold", type=float, help="A threshold to stop calculation during auto compute of UPVC filter", default = 1.0)
parser.add_argument("-a", dest="enable_stat", help="Enable auto compute of UPVC filter", action="store_true")
...
def compute_best_filter_for_depth_per_score(
        ...
        tp_stat_depth_per_sc, fp_stat_depth_per_sc,
        percentage, score,
...
def compute_best_filter_for_depth(tp_stat, fp_stat, tp, depth_filter):
    ...
    for percentage in range(100, 0, -1):
        ...
        for score in range(10, 41):
            tp_stat_depth_per_sc = \
                [val for (per, sc), val in tp_stat.items()
                 if per == percentage and sc == score]
            fp_stat_depth_per_sc = \
                [val for (per, sc), val in fp_stat.items()
                 if per == percentage and sc == score]
...
def compute_best_filter(stat):
    ...
    for d in range(depth_limit):
        ...
        filter, tp_f, fp_f = compute_best_filter_for_depth(tp_stat, fp_stat, tp, d)
        tp_filtered += tp_f
        fp_filtered += fp_f
        print("%d\t(%d,\t%d)\t(%.2f%%,\t%.2f%%)" % (d, filter[0], filter[1], per(tp_f, tp_f + fp_f), per(tp_f, tp)))
    print("--------------------------------------------------------")
