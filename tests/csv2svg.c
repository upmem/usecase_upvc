...
static void add_point2(struct line_head *line_list,
                       struct line_head *first_point_head,
                       line_t *first_point,
                       type_t type,
                       double time,
                       double value,
...
static void add_point1(struct line_head *first_point_head,
                       double time,
                       double value,
                       double *width_offset)
{
...
static void add_point(struct line_head *line_list,
                      struct line_head *first_point_head,
                      type_t type,
                      double time,
                      double value,
...
static bool read_line(FILE *fp,
                      char *str,
                      struct line_head *line_list,
                      struct line_head *first_point,
                      unsigned int *width,
                      unsigned int *height,
                      double *width_offset)
{
...
static void generate_svg(FILE *fp,
                         char *str,
                         struct line_head *line_list,
                         unsigned int width,
                         unsigned int height)
{
        ...
        sprintf(str, "<svg xmlns=\"http://www.w3.org/2000/svg\" xmlns:xlink=\"http://www.w3.org/1999/xlink\" version=\"1.1\" width=\"%u\" height=\"%u\">\n", width*ZOOM, height*ZOOM);
        fwrite(str, sizeof(char), strlen(str), fp);
        ...
        SLIST_FOREACH(current_line, line_list, next) {
                sprintf(str,
                        ...
                        current_line->y1*ZOOM,
                        current_line->x2*ZOOM,
                        current_line->y2*ZOOM,
                        type2string[current_line->type],
                        0.01*ZOOM);
                fwrite(str, sizeof(char), strlen(str), fp);
        }
